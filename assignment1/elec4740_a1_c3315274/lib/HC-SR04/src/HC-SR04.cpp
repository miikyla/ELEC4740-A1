...
float HC_SR04::distCM()
{
    ...
    Serial.printf("uSecondsToCM %f\n", uSecondsToCM);
    unsigned long timeUntilLow = triggerAndMeasurePulse();
    Serial.printf("timeUntilLow %ld\n", timeUntilLow);
...
unsigned long HC_SR04::triggerAndMeasurePulse()
{
    ...
    ATOMIC_BLOCK() {
        ...
        while (pinReadFast(echoPin) != HIGH) {
            ...
            Serial.printf("xx echo high, duration is: %ld\n", duration);
            if (duration >= timeoutHigh) {
                // Didn't recieve a pulse
                Serial.printf("--------------------------Didn't receive pulse\n");
                return 0;
            }
        }
        ...
        while (pinReadFast(echoPin) != LOW) {
            duration = micros() - start;
            Serial.printf("xx echo low, duration is: %ld\n", duration);
            if (duration >= timeoutLow) {
                // Pulse lasted longer than the 4 meter range limit
                Serial.printf("-----------------Pulse longer than 4m\n");
                return 0;
            }
        }
        Serial.printf("!!!!!!!!!!!duration is %ld\n", duration);
        return duration;
    }

    Serial.printf("returning null\n");
    return NULL;
}
